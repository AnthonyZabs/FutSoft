<?php 
    ...
    function userD($varID,$varW)
    {
        ...
        $useW       =   MYSQL_::Bits("SELECT * FROM users WHERE id = '$usID' ");
    ...
    function userR($var)
    {
        ...
        $usbusq     =   MYSQL_::Bits("SELECT * FROM users WHERE id = '$usID' ");
        $usB        =   mysqli_fetch_assoc($usbusq);

        $usrank     =   $usB['rank'];

        //Buscamos ahora el nombre del rango
        $rankbusq   =   MYSQL_::Bits("SELECT * FROM ranks WHERE rank = '$usrank' ");
